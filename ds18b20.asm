...
; This file was generated Thu Jul 20 15:21:31 2017
;--------------------------------------------------------
	.module ds18b20
	.optsdcc -mstm8
...
;	ds18b20.c: 490: t = (((temp & 0xf0) >> 4) + (temp & 0x07) * 0.125); 
	...
	ldw	(0x03, sp), x
	ldw	(0x01, sp), y
;	ds18b20.c: 491: temp = DS18B20_ReadByte();
	call	_DS18B20_ReadByte
;	ds18b20.c: 492: t += ((temp & 0x0f) << 4);
	and	a, #0x0f
	clrw	x
	ld	xl, a
	sllw	x
	...
	call	___fsadd
	addw	sp, #8
;	ds18b20.c: 493: return t;
	addw	sp, #14
	ret
;	ds18b20.c: 499: int main () {
;	-----------------------------------------
;	 function main
;	-----------------------------------------
_main:
	sub	sp, #22
;	ds18b20.c: 503: InitializeSystemClock();
	call	_InitializeSystemClock
;	ds18b20.c: 506: PD_DDR = (1 << 3) | (1 << 2); // output mode
	ldw	x, #0x5011
	ld	a, #0x0c
	ld	(x), a
;	ds18b20.c: 507: PD_CR1 = (1 << 3) | (1 << 2); // push-pull
	ldw	x, #0x5012
	ld	a, #0x0c
	ld	(x), a
;	ds18b20.c: 508: PD_CR2 = (1 << 3) | (1 << 2); // up to 10MHz speed
	ldw	x, #0x5013
	ld	a, #0x0c
	ld	(x), a
;	ds18b20.c: 509: tm1637Init();
	call	_tm1637Init
;	ds18b20.c: 511: InitializeUART();
	call	_InitializeUART
;	ds18b20.c: 515: while (1) {
00114$:
;	ds18b20.c: 518: objTemp = DS18B20_ReadTemperature() * 100; 
	...
	ldw	(0x09, sp), x
	ldw	(0x07, sp), y
;	ds18b20.c: 521: while (objTemp > 1000) {
	clrw	x
	ldw	(0x0b, sp), x
00101$:
	clrw	x
	...
	tnz	a
	jreq	00127$
;	ds18b20.c: 522: vierde+=1;
	ldw	x, (0x0b, sp)
	incw	x
	ldw	(0x0b, sp), x
;	ds18b20.c: 523: objTemp-=1000;
	clrw	x
	pushw	x
	push	#0x7a
	push	#0x44
	...
	ldw	(0x07, sp), y
	jra	00101$
;	ds18b20.c: 525: while (objTemp > 100) {
00127$:
	ldw	y, (0x0b, sp)
	ldw	(0x15, sp), y
	clrw	x
...
00104$:
	...
	tnz	a
	jreq	00128$
;	ds18b20.c: 526: derde+=1;
	ldw	x, (0x03, sp)
	incw	x
	ldw	(0x03, sp), x
;	ds18b20.c: 527: objTemp-=100;
	clrw	x
	pushw	x
	push	#0xc8
	push	#0x42
	...
	ldw	(0x07, sp), y
	jra	00104$
;	ds18b20.c: 529: while (objTemp > 10) {
00128$:
	ldw	y, (0x03, sp)
	ldw	(0x13, sp), y
	clrw	x
...
00107$:
	...
	tnz	a
	jreq	00129$
;	ds18b20.c: 530: tweede+=1;
	ldw	x, (0x01, sp)
	incw	x
	ldw	(0x01, sp), x
;	ds18b20.c: 531: objTemp-=10;
	clrw	x
	pushw	x
	push	#0x20
	push	#0x41
...
00110$:
	...
	tnz	a
	jreq	00112$
;	ds18b20.c: 535: eerste+=1;
	ldw	x, (0x05, sp)
	incw	x
	ldw	(0x05, sp), x
;	ds18b20.c: 536: objTemp-=1;
	clrw	x
	pushw	x
	push	#0x80
	push	#0x3f
	...
	jra	00110$
00112$:
;	ds18b20.c: 539: utemp=vierde*1000+derde*100+tweede*10+eerste;
	ldw	x, (0x15, sp)
	pushw	x
	push	#0xe8
	push	#0x03
	...
	decw	y
00162$:
;	ds18b20.c: 542: tm1637DisplayDecimal(utemp, 1); // eg 37:12
	push	#0x01
	push	#0x00
	pushw	x
	pushw	y
